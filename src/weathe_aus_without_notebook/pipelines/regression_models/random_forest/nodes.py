"""
Nodos para el pipeline de Random Forest Regressor.
"""
...
def train_random_forest(modeling_data_regression: pd.DataFrame, parameters: Dict) -> None:
    """
    ...
    n_estimators = parameters.get("train", {}).get("n_estimators", 100)
    max_depth = parameters.get("train", {}).get("max_depth", None)
    min_samples_split = parameters.get("train", {}).get("min_samples_split", 2)
    min_samples_leaf = parameters.get("train", {}).get("min_samples_leaf", 1)
    ...
    logger.info(f"Entrenando modelo Random Forest con {n_estimators} árboles...")
    model.fit(X_train, y_train)
    
    # Obtener importancia de características
    feature_importances = dict(zip(X_train.columns, model.feature_importances_))
    
    logger.info("Características más importantes:")
    for feature, importance in sorted(feature_importances.items(), key=lambda x: x[1], reverse=True)[:10]:
        logger.info(f"  {feature}: {importance:.4f}")
    
    # Guardar modelo usando joblib
    model_path = "data/06_models/random_forest_model.joblib"
    ...
    plt.title(f'Random Forest: Valores Reales vs Predicciones\nR² = {metrics["r2"]:.4f}, RMSE = {metrics["rmse"]:.4f}')
    ...
    # Guardar el gráfico
    plt.tight_layout()
    plt.savefig('data/08_reporting/random_forest_scatter.png')
    plt.close()
    ...
    # Guardar el gráfico
    plt.tight_layout()
    plt.savefig('data/08_reporting/random_forest_residuals.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('data/08_reporting/random_forest_feature_importance.png')
    plt.close()
    
    # Guardar resumen de métricas en un archivo JSON
    metrics_file = 'data/08_reporting/random_forest_metrics.json'
    ...
    importance_file = 'data/08_reporting/random_forest_feature_importance.csv'
